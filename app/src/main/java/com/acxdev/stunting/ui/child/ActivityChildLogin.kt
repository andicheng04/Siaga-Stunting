...
import com.acxdev.stunting.databinding.ActivityChildLoginBinding
                    ...
                    user.addListenerForSingleValueEvent(object : ValueEventListener {
                        override fun onDataChange(p0: DataSnapshot) {
                            if(p0.child(binding.mail.child()).exists()){
                                ...
                                putPrefs(Constant.GENDER, users.gender)
                                startActivity(Intent(this@ActivityChildLogin, ActivityChild3::class.java).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP))
                                finishAffinity()
                            }
                            else{
                                binding.progress.visibility = View.GONE
