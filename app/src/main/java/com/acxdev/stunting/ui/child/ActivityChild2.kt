...
import com.acxdev.stunting.databinding.ActivityChild2Binding
...
class ActivityChild2 : AppCompatActivity() {
    private lateinit var binding: ActivityChild2Binding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityChild2Binding.inflate(layoutInflater)
        ...
        binding.next.setOnClickListener {
            if(binding.mail.alertEmpty(Language.ID) && binding.phone.alertEmpty(Language.ID) && binding.password.alertEmpty(Language.ID)){
                val users = User(data.name, data.birthDate, data.gender, binding.mail.editText!!.text.toString(), binding.phone.editText!!.text.toString(), null, "child")
                    ...
                    user.addListenerForSingleValueEvent(object : ValueEventListener {
                        override fun onDataChange(p0: DataSnapshot) {
                            ...
                            if(p0.child(binding.mail.child()).exists()){
                                ...
                                startActivity(Intent(this@ActivityChild2, ActivityChildLogin::class.java))
                            }
                        }
